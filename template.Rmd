...
hailData <- shp %>%
  select(HAIL_RISKS,NRI_ID)

hailData2 <- st_transform(hailData, crs = 4326)
pal <- colorNumeric(
  palette = c("white", "red"),   # gradient from white to red
  domain = hailData2$HAIL_RISKS  # variable to map
)

# Build leaflet map
map_leaflet <- leaflet(hailData2) %>%
  addTiles() %>%
  addPolygons(
    fillColor = ~pal(HAIL_RISKS),   # use palette function
    color = "black",                # border color
    weight = 0.5,
    fillOpacity = 0.7,
    popup = ~paste0(HAIL_RISKS)
  ) %>%
  addLegend(
    pal = pal,
    values = ~HAIL_RISKS,
